...
/**
 * Script CLI pour récupérer le JSON d'un live Kick via ytdlp.online
 * Usage: php kick_json.php <URL_kick.com>
 *    ou: php kick_json.php  (saisie interactive)
 */
...
if ($argc >= 2) {
    ...
    echo "┌─────────────────────────────────────┐\n";
    echo "│       Kick JSON Fetcher (yt-dlp)    │\n";
    echo "└─────────────────────────────────────┘\n\n";
    echo "🔗 Entre l'URL Kick (ex: https://kick.com/miithyx) :\n> ";
...
// ─── Validation ───────────────────────────────────────────────────────────────
if (!preg_match('#^https?://(www\.)?kick\.com/#i', $url)) {
    echo "❌ L'URL doit commencer par https://kick.com/\n";
    echo "   Reçu : {$url}\n";
    exit(1);
}
...
$outputFile = __DIR__ . DIRECTORY_SEPARATOR . "kick_{$channel}.json";
...
// Détecter si un flux HLS est présent
$hasHls = false;
$qualities = [];
...
if (!empty($raw['formats'])) {
    foreach ($raw['formats'] as $fmt) {
        $fmtUrl = $fmt['url'] ?? '';
        if (!empty($fmtUrl)) {
            // Vérifier présence HLS
            if (str_contains($fmtUrl, '.m3u8') || ($fmt['protocol'] ?? '') === 'm3u8_native') {
                $hasHls = true;
            }
            // Construire la liste des qualités
            $qualities[] = [
